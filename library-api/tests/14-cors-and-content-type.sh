H=$(curl -s -D - -o /dev/null -X OPTIONS "$BASE/Authors/Update" -H 'Origin: http://localhost:3000' -H 'Access-Control-Request-Method: PATCH' -H 'Access-Control-Request-Headers: content-type')
...
expect_match "preflight allows PATCH" "$(hdr access-control-allow-methods)" 'patch|\*'
