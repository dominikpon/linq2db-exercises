...
public partial class AuthorsController(LibraryDatabase db) : ControllerBase
{
    ...
    public class GetAllTests : LibraryTest
    {
        ...
        public void Is_ordered_by_last_name_then_first_name()
        {
            ...
            Assert.Equal(names.OrderBy(n => n.LastName, StringComparer.Ordinal).ThenBy(n => n.FirstName, StringComparer.Ordinal), names);
        }
    }
    ...
    public class GetPageTests : LibraryTest
    {
        ...
        {
            Assert.Empty(AuthorsController.GetPage(99, 10));
        }

        [Fact]
        public void Throws_on_nonsense_paging()
        {
            Assert.Throws<ValidationException>(() => AuthorsController.GetPage(0, 10));
    ...
    public class GetSortedTests : LibraryTest
    {
        ...
        {
            var result = AuthorsController.GetSorted(AuthorSort.Created, descending: true);
    ...
    public class GetFilteredTests : LibraryTest
    {
        ...
        {
            var result = AuthorsController.GetFiltered(bornAfter: new DateOnly(1980, 1, 1), bornBefore: new DateOnly(1990, 1, 1));
            Assert.All(result, a => Assert.InRange(a.BirthDate!.Value, new DateOnly(1980, 1, 1), new DateOnly(1990, 1, 1)));
        }

        [Fact]
        public void Throws_when_the_range_is_inverted()
        {
            Assert.Throws<ValidationException>(() => AuthorsController.GetFiltered(bornAfter: new DateOnly(2000, 1, 1), bornBefore: new DateOnly(1990, 1, 1)));
        }
    }
